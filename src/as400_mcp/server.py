...
# =============================================================================
#
# IMPORTANT: MCP uses stdin/stdout for JSON-RPC. Do NOT print() debugging output,
# or you'll corrupt the protocol. Use this file logger instead.
#
# Configure via env:
#   AS400_MCP_LOG           Path to log file (e.g. C:\TEMP\as400-mcp.log)
#   AS400_MCP_LOG_LEVEL     DEBUG|INFO|WARNING|ERROR  (default: INFO)
#   AS400_MCP_LOG_STDERR    1 to also emit WARNING+ to stderr (default: off)
#   AS400_MCP_LOG_SQL       1 to include SQL previews in logs (default: 1, truncated)
#   AS400_MCP_SQL_TIMEOUT   Query timeout (seconds) applied to cursor.timeout (optional)
#   AS400_MCP_CONNECT_TIMEOUT  Connect timeout (seconds) passed to pyodbc.connect (optional)
#
_logger = logging.getLogger("as400_mcp")

_current_tool = contextvars.ContextVar("as400_mcp_tool", default=None)
_current_call_id = contextvars.ContextVar("as400_mcp_call_id", default=None)
...
def _truncate(value: Any, limit: int = 400) -> Any:
    """Truncate big values for logs without changing the real data flow."""
...
def _redact_connection_string(conn_str: str) -> str:
    # Mask common password keys while keeping useful debugging context.
    redacted = re.sub(r"(?i)\b(PWD|PASSWORD)\s*=\s*[^;]*", r"\1=***", conn_str)
    return redacted


def _conn_summary(conn_str: str) -> dict[str, Any]:
    """
    Parse a minimal summary from an ODBC connection string.
    Returns only non-sensitive parts for logging.
    """
    parts: dict[str, Any] = {}
    for kv in conn_str.split(";"):
        ...
        if k in {"PWD", "PASSWORD"}:
            continue
        if k in {"DRIVER", "SYSTEM", "UID", "CCSID"}:
            parts[k] = v
    return parts
...
def configure_logging() -> None:
    ...
    level_name = os.environ.get("AS400_MCP_LOG_LEVEL", "INFO").upper().strip()
    level = getattr(logging, level_name, logging.INFO)
    _logger.setLevel(level)

    log_path = os.environ.get("AS400_MCP_LOG")
    if not log_path:
        # Default: next to this file.
        log_path = os.path.join(os.path.dirname(__file__), "as400-mcp.log")
    ...
    try:
        ...
        # If file logging fails, fall back to stderr so we still capture something.
        sh = logging.StreamHandler()
        sh.setFormatter(fmt)
        _logger.addHandler(sh)
        _logger.warning("Failed to initialize file logging. Falling back to stderr.")

    if _truthy_env("AS400_MCP_LOG_STDERR", default=False):
        sh = logging.StreamHandler()
    ...
    try:
        if isinstance(result, dict):
            ...
            if "rows" in result and isinstance(result["rows"], list):
                return f"dict keys={keys} rows={len(result['rows'])}"
            if "source_lines" in result and isinstance(result["source_lines"], list):
                return f"dict keys={keys} source_lines={len(result['source_lines'])}"
...
def log_tool(func):
    """
    Decorator for stdio-safe tool tracing.
    Logs START/END/ERROR with a per-call id and duration.
    """
    @wraps(func)
    def wrapper(*args, **kwargs):
        configure_logging()
        call_id = uuid.uuid4().hex[:10]
        token_tool = _current_tool.set(func.__name__)
        token_id = _current_call_id.set(call_id)

        try:
            bound = inspect.signature(func).bind_partial(*args, **kwargs)
            bound.apply_defaults()
            arg_map = {k: _truncate(v) for k, v in bound.arguments.items()}
        except Exception:
            arg_map = {"args": _truncate(args), "kwargs": _truncate(kwargs)}

        t0 = time.perf_counter()
        _logger.info("TOOL START name=%s id=%s args=%s", func.__name__, call_id, arg_map)
        try:
            res = func(*args, **kwargs)
            dt = time.perf_counter() - t0
            _logger.info(
                "TOOL END   name=%s id=%s elapsed=%.3fs result=%s",
                func.__name__,
                call_id,
                dt,
            ...
            dt = time.perf_counter() - t0
            _logger.exception("TOOL ERROR name=%s id=%s elapsed=%.3fs", func.__name__, call_id, dt)
            raise
        finally:
            _current_tool.reset(token_tool)
            _current_call_id.reset(token_id)

    return wrapper


def odbc_execute(cursor: pyodbc.Cursor, sql: str, params=None) -> pyodbc.Cursor:
    """
    Thin wrapper around cursor.execute() that logs timing.
    Designed to help pinpoint "hang" points (connect/execute/fetch).
    """
    configure_logging()
    tool = _current_tool.get()
    call_id = _current_call_id.get()
    log_sql = _truthy_env("AS400_MCP_LOG_SQL", default=True)
    sql_preview = _truncate(sql, limit=500) if log_sql else "<sql hidden>"

    q_timeout = _get_int_env("AS400_MCP_SQL_TIMEOUT")
    if q_timeout is not None:
        try:
            cursor.timeout = q_timeout
        except Exception:
            pass

    t0 = time.perf_counter()
    _logger.debug("SQL EXEC start tool=%s id=%s sql=%s", tool, call_id, sql_preview)
    try:
        if params is None:
            cursor.execute(sql)
        else:
        ...
        dt = time.perf_counter() - t0
        _logger.debug("SQL EXEC ok    tool=%s id=%s elapsed=%.3fs", tool, call_id, dt)
        return cursor
    except Exception:
        dt = time.perf_counter() - t0
        _logger.exception("SQL EXEC fail  tool=%s id=%s elapsed=%.3fs sql=%s", tool, call_id, dt, sql_preview)
        raise


def odbc_fetchall(cursor: pyodbc.Cursor):
    configure_logging()
    tool = _current_tool.get()
    call_id = _current_call_id.get()
    t0 = time.perf_counter()
    _logger.debug("SQL FETCHALL start tool=%s id=%s", tool, call_id)
    try:
        rows = cursor.fetchall()
        dt = time.perf_counter() - t0
        _logger.debug("SQL FETCHALL ok    tool=%s id=%s elapsed=%.3fs rows=%s", tool, call_id, dt, len(rows))
        return rows
    except Exception:
        dt = time.perf_counter() - t0
        _logger.exception("SQL FETCHALL fail  tool=%s id=%s elapsed=%.3fs", tool, call_id, dt)
        raise


def odbc_fetchone(cursor: pyodbc.Cursor):
    configure_logging()
    tool = _current_tool.get()
    call_id = _current_call_id.get()
    t0 = time.perf_counter()
    _logger.debug("SQL FETCHONE start tool=%s id=%s", tool, call_id)
    try:
        row = cursor.fetchone()
        dt = time.perf_counter() - t0
        _logger.debug("SQL FETCHONE ok    tool=%s id=%s elapsed=%.3fs has_row=%s", tool, call_id, dt, row is not None)
        return row
    except Exception:
        dt = time.perf_counter() - t0
        _logger.exception("SQL FETCHONE fail  tool=%s id=%s elapsed=%.3fs", tool, call_id, dt)
        raise
...
def get_connection() -> pyodbc.Connection:
    ...
    # Connect is a common "hang" point: log before/after.
    tool = _current_tool.get()
    call_id = _current_call_id.get()
    redacted = _redact_connection_string(CONNECTION_STRING)
    conn_info = _conn_summary(CONNECTION_STRING)

    t0 = time.perf_counter()
    _logger.debug("CONNECT start tool=%s id=%s info=%s conn=%s", tool, call_id, conn_info, redacted)
    try:
        connect_timeout = _get_int_env("AS400_MCP_CONNECT_TIMEOUT")
        ...
        dt = time.perf_counter() - t0
        _logger.debug("CONNECT ok    tool=%s id=%s elapsed=%.3fs info=%s", tool, call_id, dt, conn_info)
        return conn
    except Exception:
        dt = time.perf_counter() - t0
        _logger.exception("CONNECT fail  tool=%s id=%s elapsed=%.3fs info=%s", tool, call_id, dt, conn_info)
        raise
...
def list_libraries(pattern: str = "%", include_system: bool = False) -> list[dict]:
    ...
    try:
        ...
        odbc_execute(cursor, sql, (pattern,))

        # 結果をdict形式に変換して返す
        columns = [desc[0] for desc in cursor.description]
        results = []
        for row in odbc_fetchall(cursor):
...
def _list_tables_internal(library: str, pattern: str = "%", table_type: str = "ALL") -> list[dict]:
    ...
    try:
        ...
        odbc_execute(cursor, sql, params)
...
def _get_columns_internal(library: str, table: str) -> list[dict]:
    ...
    try:
        ...
        odbc_execute(cursor, sql, (library.upper(), table.upper()))
...
def _list_source_files_internal(library: str, pattern: str = "%") -> list[dict]:
    ...
    try:
        ...
        odbc_execute(cursor, sql, [library.upper(), pattern])
...
def list_sources(library: str, source_file: str = "QCLSRC", pattern: str = "%") -> list[dict]:
    ...
    try:
        ...
        odbc_execute(cursor, sql, (library.upper(), source_file.upper(), pattern))
...
def _get_source_internal(library: str, source_file: str, member: str) -> dict:
    ...
    try:
        ...
        odbc_execute(cursor, meta_sql, (library.upper(), source_file.upper(), member.upper()))
        meta_row = odbc_fetchone(cursor)
        ...
        source_sql = f"""
            SELECT
                ...
                SRCDAT,  -- 更新日（YYMMDD形式）
                CAST(
                CAST(SRCDTA AS VARCHAR(112) CCSID 284)
                AS VARCHAR(112) CCSID 1208
              ) AS SRCDTA   -- ソース行
            FROM
                {alias_name}
            ORDER BY
        ...
        odbc_execute(cursor, source_sql)

        # 各行を構造化して格納
        lines = []
        for row in odbc_fetchall(cursor):
            
            raw = row[2] if row[2] is not None else ""
            if isinstance(raw, memoryview):
                raw = raw.tobytes()
            if isinstance(raw, (bytes, bytearray)):
                raw = raw.decode("utf-8", errors="replace")

            text_line = str(raw).rstrip()  # mantiene espacios a la izquierda (indentación)

            lines.append(
                {
                    "seq": float(row[0]) if row[0] else 0,  # 行番号（小数点付き）
                    "date": str(row[1]) if row[1] else "",  # 更新日（YYMMDD形式）
                    "text": text_line,  # ソース行（トリム）
                }
            )
...
def _get_table_info_internal(library: str, table: str) -> dict:
    ...
    try:
        ...
        odbc_execute(cursor, table_sql, (library.upper(), table.upper()))
        table_row = odbc_fetchone(cursor)
        ...
        odbc_execute(cursor, key_sql, (library.upper(), table.upper()))
        keys = [row[0].strip() for row in odbc_fetchall(cursor)]
            ...
            try:
                dds_key_sql = """
                    ...
                    ORDER BY
                        DBKORD
                """
                odbc_execute(cursor, dds_key_sql, (library.upper(), table.upper()))
                keys = [row[0].strip() for row in odbc_fetchall(cursor)]
            except Exception:
        ...
        odbc_execute(cursor, index_sql, (library.upper(), table.upper()))
        index_cols = [desc[0] for desc in cursor.description]
        indexes = [strip_values(dict(zip(index_cols, row))) for row in odbc_fetchall(cursor)]
...
def get_system_info() -> dict:
    ...
    try:
        ...
        try:
            odbc_execute(cursor, ver_sql)
            row = odbc_fetchone(cursor)
            if row:
                result["version"] = {
        ...
        try:
            odbc_execute(cursor, compiler_sql)
            compilers = []
            for row in odbc_fetchall(cursor):
                compilers.append(
                    {
...
def get_program_references(library: str, program: str) -> dict:
    ...
    try:
        ...
        try:
            odbc_execute(cursor, file_sql, (library.upper(), program.upper()))
            for row in odbc_fetchall(cursor):
        ...
        if not use_source_analysis:
            ...
            """
            try:
                odbc_execute(cursor, call_sql, (library.upper(), program.upper()))
                for row in odbc_fetchall(cursor):
        ...
        if use_source_analysis:
            ...
            src_sql = """
                ...
                WHERE
                    OBJNAME = ?
            """
            odbc_execute(cursor, src_sql, (library.upper(), program.upper()))
            src_row = odbc_fetchone(cursor)
...
# --- SQL error handling helpers ------------------------------------------------

# Common DB2 for i error hints (best-effort).
_DB2_ERROR_HINTS: dict[str, str] = {
    "SQL0104": (
        "Error de sintaxis. En IBM i SQL no puedes referenciar un *member* así: "
        "LIB.FILE(MBR). Para leer un member usa la tool get_source, o un ALIAS/OVRDBF "
        "(no disponible a través de execute_sql si solo permites SELECT)."
    ),
    "SQL0332": (
        "Error de conversión de caracteres/CCSID. Prueba a castear el campo con el CCSID "
        "correcto (p.ej. CAST(col AS VARCHAR(...) CCSID 284/37/500)) o doble CAST a 1208."
    ),
    "SQL0204": (
        "Objeto no encontrado. Revisa esquema/biblioteca y nombre (IBM i suele requerir MAYÚSCULAS)."
    ),
    "SQL0551": (
        "Sin autorización. Revisa permisos del usuario sobre el objeto/esquema o la vista QSYS2."
    ),
}

# Strip string literals to avoid false positives inside quoted paths, etc.
_SQL_STRING_LITERAL_RE = re.compile(r"(?s)'(?:''|[^'])*'")

# Common LLM mistake for IBM i: using FILE(MEMBER) directly as a table name in FROM/JOIN.
# Example (INVALID in Db2 for i SQL):  FROM LIBMCP.QCLSRC(MCCA136)
# This regex is intentionally narrow so it doesn't flag valid table-functions like:
#   FROM TABLE(QSYS2.IFS_READ('...'))
_MEMBER_IN_FROM_RE = re.compile(
    r"(?is)\b(?:FROM|JOIN)\s+(?!TABLE\b)(?:[A-Z#$@][A-Z0-9_#$@]*\.)?[A-Z#$@][A-Z0-9_#$@]*\s*\(\s*[A-Z#$@][A-Z0-9_#$@]*\s*\)"
)


def _extract_db2_error(exc: Exception) -> dict[str, Any]:
    """Extract SQLSTATE / SQLxxxx / native code from a pyodbc exception."""
    sqlstate = None
    message = ""
    native_code = None
    ...
    if isinstance(exc, pyodbc.Error):
        try:
            if exc.args:
                # Typical shape: ('42000', '[42000] ... SQL0104 ... (-104) ...')
    ...
    if m2:
        try:
            native_code = int(m2.group(1))
        except Exception:
            native_code = None

    hint = None
    if sqlcode and sqlcode in _DB2_ERROR_HINTS:
        hint = _DB2_ERROR_HINTS[sqlcode]
    elif sqlstate and str(sqlstate).startswith("08"):
        hint = (
            "Fallo de comunicación/timeout con IBM i. Revisa red/host/credenciales y considera "
            "AS400_MCP_CONNECT_TIMEOUT / AS400_MCP_SQL_TIMEOUT."
        )
...
def _tool_error_result(sql: str, *, error: dict[str, Any], params: list | None = None) -> dict[str, Any]:
    """Return a *successful* tool response that encodes an error (prevents client retries/hangs)."""
    call_id = _current_call_id.get()
    return {
        "ok": False,
        "error": error,
        "debug_id": call_id,
        "sql": _truncate(sql, limit=2000),
        "params": _truncate(params or []),
    }


def _validate_select_only(sql: str) -> dict[str, Any] | None:
    # Security: only allow a single SELECT statement.
    s = sql.strip()
    s_upper = s.upper()
    ...
    if not s_upper.startswith("SELECT"):
        return {
            ...
            "hint": "execute_sql es de solo lectura; usa SELECT únicamente.",
        }

    # Reject obvious multi-statement attempts.
    if ";" in s:
        # Very conservative: easiest safe rule. If you need semicolons in strings, relax later.
        return {
            "type": "VALIDATION_ERROR",
            "message": "Semicolons are not allowed (single statement only).",
            "hint": "Envía un solo SELECT sin ';'.",
        }

    # Common LLM mistake for IBM i: FILE(MEMBER) in FROM.
    s_scan = _SQL_STRING_LITERAL_RE.sub("''", s_upper)
    if _MEMBER_IN_FROM_RE.search(s_scan):
        return {
            "type": "SQL_SYNTAX_HINT",
            "message": "IBM i SQL does not support selecting from a member using FILE(MBR) syntax.",
            "hint": _DB2_ERROR_HINTS["SQL0104"],
        }

    return None


def _infer_execute_sql_warnings(sql: str, columns: list[str], rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Heuristics to help LLMs/users when results look empty due to LOB/CLOB handling.

    We keep this conservative (warnings only) so it doesn't block valid queries.
    """
    ...
    try:
        ...
        # QSYS2.IFS_READ* returns LINE as CLOB; many ODBC clients show it as empty unless casted.
        if "QSYS2.IFS_READ" in s_scan and "LINE" in [c.upper() for c in columns]:
            # If caller already casts/transforms LINE, don't warn.
            if not re.search(r"(?is)\b(VARCHAR|CHAR|SUBSTR|RTRIM|TRIM)\s*\(\s*LINE\b|\bCAST\s*\(\s*LINE\s+AS\s+VARCHAR", s_scan):
                total = len(rows)
                if total >= 5:
                    empty = 0
                    for r in rows:
                        ...
                        if v is None:
                            ...
                            empty += 1
                    if empty / max(total, 1) >= 0.7:
                        warnings.append(
                            {
                                "type": "CLOB_DISPLAY_HINT",
                                "message": "La columna LINE puede venir como CLOB (p.ej. QSYS2.IFS_READ*). Algunos drivers ODBC la devuelven vacía si no la casteas.",
                                "hint": (
                                    "Prueba a castear: SELECT LINE_NUMBER, VARCHAR(LINE, 112) AS LINE "
                                    "FROM TABLE(QSYS2.IFS_READ_UTF8(...)) ...; "
                                    "o ajusta MAXIMUM_LINE_LENGTH / END_OF_LINE."
                                ),
                            }
                        )
    except Exception:
        # Never fail the tool due to warning heuristics.
        pass

    return warnings


def _execute_sql_internal(sql: str, params: list | None = None, max_rows: int = 1000) -> dict:
    """Execute a single SELECT statement (read-only) with robust error handling.

    IMPORTANT: This function returns a *normal* tool response even on SQL errors
    (ok=False + error payload) so clients like Windsurf don't get stuck retrying.
    """
    configure_logging()

    validation = _validate_select_only(sql)
    if validation is not None:
        return _tool_error_result(sql, error=validation, params=params)
    ...
    if max_rows_i < 0:
        max_rows_i = 0
    # Avoid accidental huge pulls that can freeze UIs.
    max_rows_i = min(max_rows_i, 5000)
    ...
    try:
        ...
        try:
            ...
            # Convert DB/ODBC errors into a structured *successful* tool result
            err = _extract_db2_error(exc)
            return _tool_error_result(sql, error=err, params=params)
        ...
        if cursor.description is None:
            return _tool_error_result(
                ...
                error={
                    "type": "NO_RESULTSET",
                    "message": "Query did not return a result set.",
                    "hint": "Asegúrate de ejecutar un SELECT que devuelva columnas.",
                },
                params=params,
            )
        ...
        # Stream results instead of fetchall() to reduce memory/UI freezes.
        remaining = max_rows_i
        while remaining > 0:
            batch_size = 200 if remaining > 200 else remaining
            try:
            ...
            for row in batch:
                ...
                for j, col in enumerate(columns):
                    value = row[j]
                    # Keep leading spaces (important for source), trim right side.
                    if isinstance(value, str):
                        value = value.rstrip()
                    # Some drivers return LOB/text as memoryview/bytes.
                    if isinstance(value, memoryview):
                        value = value.tobytes()
                    if isinstance(value, (bytes, bytearray)):
                        # Best-effort decode. If the content is not UTF-8, it will be visible,
                        # and callers can CAST with the right CCSID.
                        value = value.decode("utf-8", errors="replace")
                    row_dict[col] = value
                rows.append(row_dict)

            remaining = max_rows_i - len(rows)
        result = {
            "ok": True,
            "columns": columns,
            "rows": rows,
            ...
            "debug_id": _current_call_id.get(),
        }

        warnings = _infer_execute_sql_warnings(sql, columns, rows)
        if warnings:
...
def main():
    ...
    # Initialize stdio-safe file logging (after .env is loaded)
    configure_logging()
    _logger.info("Server start name=as400-mcp version=3.0.0 python=%s pyodbc=%s", sys.version.split()[0], getattr(pyodbc, '__version__', 'unknown'))
    ...
    _logger.info("AS400 connection configured info=%s", _conn_summary(CONNECTION_STRING))
